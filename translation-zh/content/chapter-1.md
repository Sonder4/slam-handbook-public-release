...
我们先考察一个简单的 SLAM 场景，以说明如何构造因子图。图 1.1 给出了该问题结构的示意图。机器人依次经过三个位姿 ${ \mathbf { \mathit { p } } } _ { 1 }$、${ \mathbf { \mathit { p } } } _ { 2 }$ 和 $p_3$，并对两个路标 $\ell_1$ 和 $\ell_2$ 进行方位观测。为了将解锚定在空间中，假设在第一个位姿 ${ \pmb p }_1$ 上还有一个绝对位置/方向测量。没有这一测量，就无法获得绝对位置的信息，因为方位测量都是相对的。<sup>1</sup>

![](images/7f2cd7879950afca0abfe3a2c35a1a703e4d6c8e28ee44484e2aef43386318b3.jpg)  
图 1.1 三个位姿和两个路标构成的玩具同步定位与建图（SLAM）示例。图中用箭头示意机器人运动，用虚线表示方位测量。
...
概率图模型利用概率分布中的结构，为复杂概率密度提供紧凑的描述机制 [592]。贝叶斯网络可能是最广为人知的图模型：它由变量节点组成，每个变量节点都关联一个先验概率密度或条件概率密度。图 1.2 展示了图 1.1 玩具示例对应的贝叶斯网络，其中明确给出了依赖于第一个位姿 $\pmb p_1$ 的初始测量 $z_1$，以及 $z_2\ldots z_4$ 四个方位测量，每个测量都同时与一个位姿和一个路标相关。按照惯例，贝叶斯网络中已知量用方形节点表示，如图所示。不过，贝叶斯网络虽然非常适合建模，下面我们还要引入另一种更适合优化的图模型；它只关注问题中的未知变量。

![](images/de0dd895a761a6a8ef2799623bc1231feeeca13f4f5f7a71c2097acf985dfe2a.jpg)  
图 1.2 图 1.1 示例对应的贝叶斯网络，其中将已知测量 $z_1\ldots z_4$ 明确表示为方形节点。
...
图 1.4 几种都可以用因子图视角表示的 SLAM 问题变体。典型的基于路标的 SLAM 同时包含位姿变量和路标变量；从位姿观测路标，并且位姿之间通常存在基于里程计的运动先验。束调整（BA）与之相同，但不包含运动先验。位姿图优化（PGO）不含路标变量，但在位姿之间增加了回环测量。最后，同时轨迹估计与建图（STEAM）与基于路标的 SLAM 类似，只是将位姿替换为更高阶状态，并采用平滑的连续时间运动先验。
...
图 1.5 一个规模更大的模拟 SLAM 示例因子图。

比图 1.1 的玩具示例更接近实际的基于路标的 SLAM 问题，其因子图可能类似图 1.5。该图通过模拟一个在平面内运动约 100 个时间步、同时观测路标的二维机器人生成。为便于可视化，每个位姿和路标都按其真实二维位置绘制。可以看到，里程计因子形成了显著的链状骨架，而在两侧则有二元<sup>2</sup>似然因子连接到约 20 个路标。在这类 SLAM 问题中，除先验之外，所有因子通常都是非线性的。
...
未知量的先验通常用高斯密度指定；在许多情况下，将测量建模为受到零均值高斯噪声污染既合理又方便。例如，给定位姿 $p$ 到给定路标 $\ell$ 的方位测量<sup>3</sup> 可建模为
...
一个重要而不太显然的观察是，式（1.18）中的因子通常只描述所涉及未知变量 $\boldsymbol\Delta_{\boldsymbol\mathcal X_i}$ 的部分概率。除简单的先验因子外，测量 $z_i$ 的维度通常低于未知量 $\boldsymbol\Delta_{\boldsymbol\mathcal X_i}$ 的维度。在这种情况下，单个因子会为 $\pmb x_i$ 定义域中的一个无限子集赋予相同的似然。例如，相机图像中的一个二维测量，与一整条能够投影到该图像位置的三维点射线相容。

尽管函数 $\pmb h_i$ 是非线性的，但如果有一个足够好的初始猜测，本章讨论的非线性优化方法通常能够收敛到式（1.18）的全局最小值。不过需要提醒的是，式（1.18）的目标函数是非凸的，因此当初始猜测较差时，无法保证不会陷入局部最小值。这促成了所谓的**可证明最优求解器**，第 6 章将对此进行介绍。下面我们先关注局部方法，而不是全局求解器；首先考虑更容易的线性化问题。
...
而 $\pmb\delta_i\triangleq\pmb x_i-\pmb x_i^0$ 是状态更新向量。这里假设 $\boldsymbol\Delta_{\boldsymbol\mathcal X_i}$ 位于向量空间中，或者等价地，可以用向量表示。当 $\pmb x$ 中的一些未知状态表示三维旋转或其他更复杂的流形类型时，情况并不总是如此。第 2 章将重新讨论这个问题。
...
这个过程称为**白化**。例如，对于标量测量，它仅仅意味着将每一项除以测量标准差 $\sigma_i$。注意，这一步消除了测量的单位（例如长度、角度），因此不同的行可以组合到同一个代价函数中。
...
这里的 Cholesky 因子 $R$ 是一个 $n\times n$ 的上三角矩阵<sup>4</sup>，通过 Cholesky 分解计算得到；Cholesky 分解是针对对称正定矩阵的 LU（下三角—上三角）分解变体。随后，先求解
...
得到 $\delta^*$。对于稠密矩阵，Cholesky 分解需要 $n^3/3$ 次浮点运算；整个算法（包括计算对称矩阵 $A^\top A$ 的一半）需要 $(m+n/3)n^2$ 次浮点运算。还可以使用 LDL<sup>\top</sup> 分解，这是 Cholesky 分解的一个变体，可以避免开平方根。
...
总之，与 SLAM 线性化优化问题相关的线性代数可以简洁地概括为：将信息矩阵 $\pmb\Lambda$ 或测量雅可比矩阵 $A$ 分解成平方根形式。由于它们都基于 SAM 问题中的矩阵平方根，我们将这类方法称为平方根 SAM，简称 SAM [249, 254]。
...
Powell 的 dogleg（PDL）算法 [885] 可以作为 LM 的更高效替代方案 [697]。Levenberg—Marquardt 算法的一个主要缺点是：如果某一步被拒绝，就必须重新分解修改后的信息矩阵，而这正是算法中最昂贵的环节。因此，PDL 的核心思想是分别计算 GN 步和 SD 步，再将二者适当地组合起来。如果 LM 步被拒绝，GN 步和 SD 步的方向仍然有效，可以用另一种方式组合，直到代价下降。因此，状态估计的每次更新只需进行一次矩阵分解，而不是多次。
...
图 1.7 图 1.1 玩具 SLAM 示例的稀疏雅可比矩阵 $A$ 的分块结构，其中空白项为零。
...
图 1.8 玩具 SLAM 问题的信息矩阵 $\pmb\Lambda$ 及其关联的无向图 $G$。

### 1.5.3 稀疏分解

我们已经看到，MAP 估计等价于求解第 1.3.3 节所述的线性方程组；对于非线性最小二乘问题，则在迭代过程中反复求解这类系统。前两节说明，$A$ 和 $A^\top A$ 都具有由因子图决定的稀疏性。无需展开细节，这种已知的稀疏模式可以显著加速 Cholesky 分解（处理 $A^\top A$ 时）或 QR 分解（处理 $A$ 时）。例如，CHOLMOD [189] 和 SuiteSparseQR [240] 都提供了高效实现，许多软件包在底层使用它们。实践中，在稀疏问题上，稀疏 Cholesky 或 LDU 分解通常也优于 QR 分解，而且优势并非只是一个常数因子。
...
仍以图 1.5 所示的较大模拟示例说明这一点。图 1.9 展示了对应稀疏雅可比矩阵 $A$ 的模式，并在右上角给出信息矩阵 $\pmb\Lambda=A^\top A$ 的模式。图右侧还给出了两种不同排序下得到的上三角 Cholesky 因子 $R$。两者都具有稀疏性，也都满足 $R^\top R=A^\top A$（变量置换除外），但稀疏程度不同，而这正决定了分解 $A$ 的代价。第一种排序最自然：先排列位姿，再排列路标，得到的稀疏 $R$ 因子有 9399 个非零项。相比之下，右下角按列近似最小度（COLAMD）启发式方法 [29, 241] 重新排序后得到的 $R$ 因子只有 4168 个非零项。然而，回代会为两种排序给出完全相同的解。
...
图 1.9 左侧是图 1.5 问题对应的测量雅可比矩阵 $A$；右侧从上到下依次为信息矩阵 $\pmb\Lambda=A^\top A$、其上三角 Cholesky 因子 $R$，以及采用更优变量排序（COLAMD）得到的替代因子 $R$。
...
存在一种通用算法，可以针对任意（最好是稀疏的）因子图，计算未知变量 $\pmb x$ 上相应的后验密度 $p(\pmb x|z)$，并将其表示为易于恢复 MAP 解的形式。如前所述，因子图以因子乘积的形式表示未归一化后验 $\phi(\pmb x)\propto p(\pmb x|\pmb z)$；在 SLAM 问题中，该图通常直接由测量生成。变量消元算法提供了一套规则，可将因子图转换为另一个称为**贝叶斯网络**的图模型，该模型只依赖未知变量 $\pmb x$，从而可以方便地进行 MAP 推断（以及采样、边缘化等其他操作）。
...
其中，$s_j$ 表示在给定变量排序 $\pmb x_1,\ldots,\pmb x_n$ 下，与变量 $\boldsymbol\Delta_{\boldsymbol\mathcal X_j}$ 关联的分隔集 $\pmb s(\pmb x_j)$ 的一次赋值。分隔集定义为消元后 $\boldsymbol\Delta_{\boldsymbol\mathcal X_j}$ 所依赖的变量集合。虽然这种分解类似链式法则，但对稀疏因子图进行消元通常会得到很小的分隔集。

消元算法每次消去一个变量 $\boldsymbol\Delta_{\boldsymbol\mathcal X_j}$，从完整因子图 $\phi_{1:n}$ 开始。当消去每个变量时，生成一个条件密度 $p(\pmb x_j|\pmb s_j)$，以及只包含剩余变量的约简因子图 $\phi_{j+1:n}$。所有变量消元后，算法返回具有所需分解形式的贝叶斯网络。

给定部分已消元的因子图 $\phi_{j:n}$，要消除单个变量 $\boldsymbol\Delta_{\boldsymbol\mathcal X_j}$，首先移除所有与该变量相邻的因子 $\phi_i(\pmb x_i)$，并将它们相乘得到乘积因子 $\psi(\pmb x_j,\pmb s_j)$。随后把 $\psi$ 分解为已消元变量上的条件分布 $p(\pmb x_j|\pmb s_j)$ 与分隔集上的新因子 $\tau(\pmb s_j)$：
...
图 1.10 玩具 SLAM 示例的变量消元：按 $\ell_1,\ell_2,p_1,p_2,p_3$ 的顺序，将图 1.3 的因子图转换为贝叶斯网络（右下）。
...
如前所述，消元算法逐个处理变量。对于每个变量 $\boldsymbol\Delta_{\boldsymbol\mathcal X_j}$，移除与之相邻的所有因子 $\phi_i(\pmb x_i)$，并形成中间乘积因子 $\psi(\pmb x_j,\pmb s_j)$。只需将所有矩阵 $A_i$ 累积到一个更大的分块矩阵 $\bar A_j$ 中即可：
...
接下来可以用多种方式对乘积 $\psi(\pmb x_j,\pmb s_j)$ 进行分解。这里讨论 QR 变体，因为它与线性化因子的联系更加直接。具体而言，与乘积因子 $\psi(\pmb x_j,\pmb s_j)$ 对应的增广矩阵 $[\bar A_j|\bar b_j]$ 可使用部分 QR 分解 [389] 重写为
...
图 1.11 将变量 $\ell_1$ 消元作为一次部分稀疏分解步骤。

使用部分 QR 分解消除单个变量的完整消元算法，等价于稀疏 QR 分解。由于上面的处理考虑了多维变量 $\pmb x_j\in\mathbb R^{n_j}$，因此实际上这是多前沿 QR 分解 [288] 的一个实例：每次同时消除多个标量变量，这有利于提高处理器利用率。在这里，标量变量按其在推断问题中的语义进行分组；而稀疏线性代数程序通常会按最大化计算效率的目标分析并分组问题。许多情况下，这两种策略高度一致。

![](images/e2e8d95287a0da35908e51410277a99b2c009af7c8e501191f52e42f89f6f4b6.jpg)  
图 1.12 玩具示例中剩余的消元步骤，完成一次完整 QR 分解。右下角的最后一步展示了最终贝叶斯网络与稀疏 Cholesky 因子 $R$ 的等价性。

### 1.6.3 作为贝叶斯网络的稀疏 Cholesky 因子
...
在每一步中，变量 $\boldsymbol\Delta_{\boldsymbol\mathcal X_j}$ 的 MAP 估计就是条件均值：
...
通过因子图运行消元算法得到的贝叶斯网络具有一个特殊性质：它是弦图（chordal graph），即任意长度大于 3 的无向环都包含一条弦，也就是连接环上两个非相邻顶点的边。在人工智能和机器学习领域，弦图更常称为三角化图。由于它仍然是贝叶斯网络，其联合密度 $p(\pmb x)$ 可以在各个变量 $\boldsymbol\Delta_{\boldsymbol\mathcal X_j}$ 上分解：

$$
p (\boldsymbol {x}) = \prod_ {j} p (\boldsymbol {x} _ {j} | \boldsymbol {\pi} _ {j}),\tag{1.54}
$$

其中，$\pi_j$ 是 $\boldsymbol\Delta_{\boldsymbol\mathcal X_j}$ 的父节点。不过，尽管贝叶斯网络是弦图，在变量层面它仍是一个非平凡图：既不是链式结构，也不是树结构。图 1.10 最后一步给出了当前玩具 SLAM 示例的弦贝叶斯网络；其中存在无向环 $\pmb p_1-\pmb p_2-\pmb\ell_1$，说明它不具备树结构。
...
重新消除受影响团时，应该采用怎样的变量排序？只有贝叶斯树受影响部分中的变量需要更新。一种策略是在受影响变量上局部应用 COLAMD。但还可以做得更好：将最近访问的变量强制放到排序末端，即放入根团。对于这种增量变量排序策略，可以使用约束 COLAMD 算法 [241]。它既能将最近访问的变量置于末端，又能保持总体上良好的排序。通常，后续更新只会影响树的一小部分，因此除大型回环外，大多数情况下都能高效完成。
