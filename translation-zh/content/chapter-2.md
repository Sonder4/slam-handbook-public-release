...
图 2.1 对于球面流形 $\mathcal M$，$T_x\mathcal M$ 是局部切平面；配合局部基，可以定义局部坐标。
...
由于旋转矩阵和变换矩阵的形式受到约束，它们并不是向量。例如，不能简单地将两个旋转矩阵相加，并期望结果仍是有效的旋转矩阵。不过，$\mathrm{SO}(d)$ 和 $\operatorname{SE}(d)$ 都属于具有额外有用结构的流形，即**矩阵 Lie 群**。我们可以利用这种结构，在因子图 SLAM 中继续执行无约束 MAP 优化（参见 Dellaert 等人 [255]、Boumal [106] 或 Barfoot [47]）。关于机器人中的 Lie 群，可参考 Chirikjian 和 Kyatkin [207] 以及 Chirikjian [205, 206] 的经典工作。
...
图 2.2 本章记号概览：wedge 算子 $(\cdot)^\wedge$ 与 vee 算子 $(\cdot)^\vee$，指数映射 $\exp(\cdot)$ 与对数映射 $\log(\cdot)$，以及便捷记号 $\operatorname{Exp}(\cdot)=\exp((\cdot)^\wedge)$ 和 $\operatorname{Log}(\cdot)=(\log(\cdot))^\vee$。图中以 $\mathrm{SO}(n)$ 为例，但这些算子对其他流形同样定义。
...
各个单独雅可比都有已知表达式 [87]。注意，每个雅可比的线性化点都考虑了扰动发生在 $\pmb\xi=\mathbf0$ 附近这一事实。
...
这样可以确保解始终位于 SE(3) 中。优化照常迭代，直到所有状态更新（包括 $\pmb\xi$）的变化足够小。式（2.19）适用于右侧位姿扰动；也可以选择左侧扰动，具体讨论见 Barfoot [47]。
...
其中 $\pmb T_w^c$ 按前述方式扰动，$\pmb H_i$ 是该误差对应的雅可比 [47]。这样，任意类型的测量都可以在因子图范式中处理。
...
闭式表达式可参阅 Barfoot [47]。下文也用 $\pmb J(\pmb\xi)$ 表示 $\operatorname{SE}(d)$ 的左雅可比，具体含义由上下文决定。Lie 群雅可比也是利用可微神经网络估计位姿的核心工具（见第 4.2 节）。
...
图 2.3 用参数化样条表示连续时间轨迹。任意时刻的位姿由具有局部支撑的已知基函数加权求和得到；每个路标测量对应一个连接 4 个系数变量和 1 个路标变量的五元因子，整体因子图仍然非常稀疏。
...
首先选择一个由白噪声驱动的线性时不变随机微分方程（SDE）：<sup>2</sup>
...
$$
Q_i=\int_{t_{i-1}}^{t_i}\Phi(t_i,s)L\mathcal QL^\top\Phi(t_i,s)^\top ds.
$$
...
此外，若想利用式（2.74）减少随机游走示例中的控制点数量，可以采用式（2.67）中针对线性样条的同一方法，因为两者最终都归结为 Lie 群控制点之间的线性插值。样条与 GP 的根本区别在于：GP 方法使用运动先验项（见图 2.6）对问题进行正则化，而样条方法不使用这种先验。<sup>4</sup>
